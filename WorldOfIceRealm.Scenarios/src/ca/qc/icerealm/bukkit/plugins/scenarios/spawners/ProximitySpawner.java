...
public class ProximitySpawner implements TimeObserver, Listener, Spawner, CoolDown {
	...
	public void timeHasCome(long time) {
		...
		if (draw && !_isCoolDownActive && _entities.size() < _maxMonster && _isPlayerAround) {
			...
			//this.logger.info(creature.toString() + " from spawner " +_startingLocation.getX() + "," + _startingLocation.getZ());
		}
		...
		if (_entities.size() == _maxMonster) {
			if (_config.DelayBeforeRespawn > 0) {
				...
				//this.logger.info("cool down timer started! at " + _startingLocation.getX() + "," + _startingLocation.getZ());
			}
			else {
				moveSpawnerToAnotherLocation();
				_isCoolDownActive = false;
				//this.logger.info("cool down is not activated, delay respawn is 0");
			}
		}
		else {
	...
	public void moveSpawnerToAnotherLocation() {
		if (_config.DelayBeforeRespawn == 0) {
			...
			_zoneServer.addListener(_activator);
			//this.logger.info("moving spawner elsewhere! at " + _startingLocation.getX() + "," + _startingLocation.getZ());
		}
	}
	...
	public void setCoolDownActive(boolean v) {
		_isCoolDownActive = v;
		//this.logger.info("cool down changed to " + v + " at " + _startingLocation.getX() + "," + _startingLocation.getZ());
	}
